...
- [ ] **S6b — Phase 06: Lead sourcing, first 100 leads** (Tue 9 - Wed 10 Sept) — Manchester done (44); **Bristol done Thu 17 Sept** (seam: only Higgihaus + Altoluxo on holidayfuture, three Guesty tells; sweep: 156 rows, 124 hosts, 24 at 5+, 20 loaded + Hopewell; Phase 07 pass run the same day: 3 Qualified (ShortStayUK on Hostaway, Cohost/StayRight Cardiff, Your Apartment OpIntel-only on Seb's override), 4 Parked, 4 Borderline, 10 DQ; reusable `S6 City Sweep — tri_angle` and `S7 CH Officer Search` workflows built, contact sweep now reports PMS tells); **Cardiff done Mon 21 Sept** (seam: 8 Hostaway sites; sweep: 152 rows, 124 hosts, 30 at 5+; 35 rows loaded with the Phase 07 pass: 5 Qualified all Hostaway-confirmed, 4 Borderline P2, 2 Parked, 7 light-tier, 17 DQ; base at 105); remaining wave 1: Liverpool, Newport, Brighton; plus LinkedIn actor pass on the named-but-unfound directors
...
- [x] **S9 — Phase 09: Scoreboard & notifications** — `phases/09-scoreboard.md` (done Tue 15 Sept; Mon 14 was missed, dates slid one day, order held) — `tools/scoreboard` renders from logs (touches win / replies tiebreak / coffee stakes / session-day streak — Seb's provisional calls, Sotirios ratifies on return); Slack ping to #sales-os wired into "log my session" via `SLACK_WEBHOOK_URL` (dry-run tested; first live ping once Seb adds the env var + allowlists `hooks.slack.com`); `tools/routines.md` written. Sotirios ON HOLIDAY — his whole setup bundle queued for his return, must land by Sun 21
- [ ] **S10a — Phase 10: Soft launch, first real sends** — `phases/10-soft-launch.md` (Wed 16 Sept — compressed to one day by the S9 slide)
...
- Both founders: set `PULSE_WEBHOOK_URL` and `PULSE_FOUNDER_EMAIL` in a Claude Code environment and allow `shonichi.app.n8n.cloud` (see `tools/pulse.md`). Seb's environment verified working S8 (13 Sept). REMAINS: Sotirios's environment (on holiday — bundle with his return setup, before Sun 21).
- Seb, before S10a fires its first ping: add `SLACK_WEBHOOK_URL` (the #sales-os incoming webhook) to the Shonichi environment and allow `hooks.slack.com`. Then set up your own daily routine per `tools/routines.md`.
