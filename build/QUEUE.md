...
- [ ] **S6b — Phase 06: Lead sourcing, first 100 leads** (Tue 9 - Wed 10 Sept) — Manchester done (44); **Bristol done Thu 17 Sept** (seam: only Higgihaus + Altoluxo on holidayfuture, three Guesty tells; sweep: 156 rows, 124 hosts, 24 at 5+, 20 loaded as `new` + Hopewell `parked`; reusable `S6 City Sweep — tri_angle` workflow built with the load rules in-workflow; Bristol cohort still needs its Phase 07 pass); remaining wave 1: Liverpool, Cardiff, Newport, Brighton; plus CH + LinkedIn contact routes for the triple-match
